...
mod integration_tests {
    ...
    fn test_integration_full_workflow() {
        ...
        assert_eq!(settings.env.get("ANTHROPIC_AUTH_TOKEN"), Some(&"sk-ant-prod".to_string()));
        ...
        assert_eq!(settings.env.get("ANTHROPIC_AUTH_TOKEN"), Some(&"sk-ant-dev".to_string()));
        assert_eq!(settings.env.get("ANTHROPIC_BASE_URL"), Some(&"https://api.dev.anthropic.com".to_string()));
    ...
    fn test_integration_settings_persistence() {
        ...
        let mut storage = ConfigStorage::default();
        let config = create_test_config("persist-test", "sk-ant-persist", "https://api.persist.com");
        ...
        settings.other.insert("test_key".to_string(), serde_json::Value::String("test_value".to_string()));
        ...
        let loaded_settings: ClaudeSettings = serde_json::from_str(&loaded_settings_content).unwrap();
        
        assert_eq!(loaded_settings.env.get("ANTHROPIC_AUTH_TOKEN"), Some(&"sk-ant-persist".to_string()));
        assert_eq!(loaded_settings.env.get("ANTHROPIC_BASE_URL"), Some(&"https://api.persist.com".to_string()));
        assert_eq!(loaded_settings.other.get("test_key"), Some(&serde_json::Value::String("test_value".to_string())));
    }
    ...
    fn test_integration_path_resolution() {
        ...
        assert_eq!(absolute_path, std::path::PathBuf::from("/absolute/path/to/claude/settings.json"));
    ...
    fn test_integration_configuration_switching_scenarios() {
        ...
        assert_eq!(settings.env.get("ANTHROPIC_BASE_URL"), Some(&"https://api1.test.com".to_string()));
    ...
    fn test_integration_custom_directory_management() {
        ...
        assert_eq!(storage.get_claude_settings_dir(), Some(&"/custom/claude/path".to_string()));
        
        // Test path resolution with custom directory
        let custom_path = get_claude_settings_path(Some("/custom/claude/path")).unwrap();
        assert_eq!(custom_path, std::path::PathBuf::from("/custom/claude/path/settings.json"));
        
        // Test changing custom directory
        storage.set_claude_settings_dir("/another/custom/path".to_string());
        
        assert_eq!(storage.get_claude_settings_dir(), Some(&"/another/custom/path".to_string()));
        
        let another_path = get_claude_settings_path(Some("/another/custom/path")).unwrap();
        assert_eq!(another_path, std::path::PathBuf::from("/another/custom/path/settings.json"));
    }
