...
mod tests {
    ...
    fn test_cli_add_command_with_model_flags() {
        let args = vec![
            ...
            "add",
            "model-config",
            "-t", "sk-ant-model-token",
            "-u", "https://model.api.com",
        ...
        match cli.command {
            Some(Commands::Add { 
                ...
                assert_eq!(small_fast_model, Some("claude-3-haiku-20240307".to_string()));
            }
            _ => panic!("Expected Add command"),
        }
    }
    ...
    fn test_cli_list_aliases_flag() {
        ...
        assert!(cli.command.is_none(), "Should have no subcommand when using --list-aliases");
    }
    ...
    fn test_cli_remove_missing_alias() {
        ...
        assert!(result.is_err(), "Should fail when no alias names provided to remove");
    }
    ...
    fn test_cli_use_missing_alias() {
        ...
        assert!(result.is_err(), "Should fail when alias name is missing for use command");
    }
    ...
    fn test_cli_completion_default_shell() {
        ...
        assert!(result.is_ok(), "Should succeed with default shell for completion command");
    ...
    fn test_cli_empty_args() {
        ...
        assert!(cli.command.is_none(), "Should have no command with empty args");
        assert!(!cli.list_aliases, "Should not have list_aliases flag");
    }

    #[test]
    fn test_cli_with_special_characters_in_alias() {
        let args = vec!["cc-switch", "add", "test-config_123", "sk-ant-test", "https://api.test.com"];
        
        let result = Cli::try_parse_from(args);
        assert!(result.is_ok(), "Should handle special characters in alias names");
    ...
    fn test_cli_unicode_in_arguments() {
        let args = vec!["cc-switch", "add", "测试-config", "sk-ant-测试", "https://αpi.测试.com"];
        
        let result = Cli::try_parse_from(args);
        assert!(result.is_ok(), "Should handle Unicode in arguments");
        
        if let Ok(cli) = result {
            if let Some(Commands::Add { alias_name, token_arg, url_arg, .. }) = cli.command {
    ...
    fn test_cli_parsing_performance() {
        ...
        assert!(duration.as_secs() < 5, "CLI parsing should be fast, took {:?}", duration);
    }
    ...
    fn test_storage_operations_integration() {
        ...
        storage.add_configuration(create_test_config("config1", "sk-ant-1", "https://api1.com"));
        storage.add_configuration(create_test_config("config2", "sk-ant-2", "https://api2.com"));
        storage.add_configuration(create_test_config("config3", "sk-ant-3", "https://api3.com"));
