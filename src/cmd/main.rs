...
fn handle_interactive_selection(storage: &ConfigStorage, custom_dir: Option<&str>) -> Result<()> {
    ...
    if raw_mode_enabled {
        ...
        if let Ok(_) = execute!(
            ...
            let result = handle_full_interactive_menu(&mut stdout, &configs, &mut selected_index, custom_dir);
...
fn handle_full_interactive_menu(
    ...
    loop {
        ...
        println!("\r{}", "║ Use ↑↓ arrows, Enter to select, Esc to cancel".dimmed());
        ...
        for (index, config) in configs.iter().enumerate() {
            if index == *selected_index {
                println!("\r> {} {}", "●".blue().bold(), config.alias_name.blue().bold());
                // Show details with better formatting
                println!("\r    Token: {}", format!("{}...{}", &config.token[..12], &config.token[config.token.len()-8..]).dimmed());
        ...
        if *selected_index == reset_index {
            println!("\r> {} {}", "●".red().bold(), "Reset to default".red().bold());
...
fn handle_simple_interactive_menu(
    ...
    for (index, config) in configs.iter().enumerate() {
        println!("{}. {} ({})", 
            ...
            format!("{}...{}", &config.token[..12], &config.token[config.token.len()-8..]).dimmed()
        );
    ...
    println!("{}. {}", configs.len() + 2, "Cancel".dimmed());
...
fn handle_selection_action(
    ...
    if selected_index < configs.len() {
        ...
        println!("Switched to configuration '{}'", selected_config.alias_name.green().bold());
        println!("Token: {}", format!("{}...{}", &selected_config.token[..12], &selected_config.token[selected_config.token.len()-8..]).dimmed());
...
fn launch_claude() -> Result<()> {
    ...
    let mut child = Command::new("claude")
        ...
        .stderr(Stdio::inherit())
        .spawn()
        .with_context(|| "Failed to launch Claude CLI. Make sure 'claude' command is available in PATH")?;
