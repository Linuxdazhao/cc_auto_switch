...
mod tests {
    ...
    fn test_config_storage_get_claude_settings_dir() {
        ...
        storage.set_claude_settings_dir("/test/path".to_string());
        assert_eq!(storage.get_claude_settings_dir(), Some(&"/test/path".to_string()));
    }
    ...
    fn test_config_storage_save_and_load() {
        ...
        let _test_get_config_storage_path = || Ok::<std::path::PathBuf, anyhow::Error>(test_config_path.clone());
        ...
        assert_eq!(loaded_storage.claude_settings_dir, Some("/custom/path".to_string()));
    ...
    fn create_test_claude_settings() -> ClaudeSettings {
        ...
        settings.env.insert("ANTHROPIC_AUTH_TOKEN".to_string(), "sk-ant-test".to_string());
        settings.env.insert("ANTHROPIC_BASE_URL".to_string(), "https://api.test.com".to_string());
        settings.other.insert("other_key".to_string(), serde_json::Value::String("other_value".to_string()));
        settings
    }
    ...
    fn test_claude_settings_serialization() {
        ...
        assert_eq!(deserialized.env.get("ANTHROPIC_AUTH_TOKEN"), settings.env.get("ANTHROPIC_AUTH_TOKEN"));
        assert_eq!(deserialized.env.get("ANTHROPIC_BASE_URL"), settings.env.get("ANTHROPIC_BASE_URL"));
        assert_eq!(deserialized.other.get("other_key"), settings.other.get("other_key"));
    }
    ...
    fn test_claude_settings_deserialization_missing_env() {
        ...
        assert!(deserialized.env.is_empty());
        assert_eq!(deserialized.other.get("other_key"), Some(&serde_json::Value::String("other_value".to_string())));
        assert_eq!(deserialized.other.get("another_key"), Some(&serde_json::Value::Number(42.into())));
    }
    ...
    fn test_claude_settings_save_and_load() {
        ...
        assert_eq!(loaded_settings.env.get("ANTHROPIC_AUTH_TOKEN"), settings.env.get("ANTHROPIC_AUTH_TOKEN"));
        assert_eq!(loaded_settings.env.get("ANTHROPIC_BASE_URL"), settings.env.get("ANTHROPIC_BASE_URL"));
        assert_eq!(loaded_settings.other.get("other_key"), settings.other.get("other_key"));
    }
    ...
    fn test_claude_settings_switch_to_config_preserves_other() {
        ...
        assert_eq!(settings.env.get("ANTHROPIC_AUTH_TOKEN"), Some(&"sk-ant-new".to_string()));
        assert_eq!(settings.env.get("ANTHROPIC_BASE_URL"), Some(&"https://api.new.com".to_string()));
        assert!(settings.other.contains_key("other_key"));
    }
    ...
    fn test_claude_settings_multiple_switches() {
        ...
        assert_eq!(settings.env.get("ANTHROPIC_AUTH_TOKEN"), Some(&"sk-ant-test1".to_string()));
        
        settings.switch_to_config(&config2);
        assert_eq!(settings.env.get("ANTHROPIC_AUTH_TOKEN"), Some(&"sk-ant-test2".to_string()));
        assert_eq!(settings.env.get("ANTHROPIC_BASE_URL"), Some(&"https://api2.test.com".to_string()));
    }
    ...
    fn test_validate_alias_name_reserved_cc() {
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "Alias name 'cc' is reserved and cannot be used");
    }
    ...
    fn test_validate_alias_name_whitespace() {
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "Alias name cannot contain whitespace");
    }
    ...
    fn test_get_claude_settings_path_absolute() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), std::path::PathBuf::from("/absolute/path/to/claude/settings.json"));
    }
    ...
    fn test_cli_parsing() {
        ...
        let args = vec!["cc-switch", "add", "my-config", "sk-ant-test", "https://api.test.com"];
        let cli = Cli::try_parse_from(args).unwrap();
        
        if let Some(Commands::Add { alias_name, token, url, force, interactive, token_arg, url_arg }) = cli.command {
    ...
    fn test_cli_parsing_with_flags() {
        ...
        let args = vec!["cc-switch", "add", "my-config", "-t", "sk-ant-test", "-u", "https://api.test.com", "-f", "-i"];
        let cli = Cli::try_parse_from(args).unwrap();
